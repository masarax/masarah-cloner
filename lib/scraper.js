...
async function scrapePage(targetUrl, targetBaseDir, framework) {
  ...
  try {
    ...
    function registerAsset(originalUrl, attrName, element) {
      ...
      element.setAttribute(attrName, localRelPath);
      assets.push({ url: absoluteUrl, dest: destPath });
      
      if (attrName === 'src' && element.tagName === 'SCRIPT') {
        jsFiles.push(destPath);
      }
    }

    root.querySelectorAll('link[rel="stylesheet"]').forEach(el => {
      registerAsset(el.getAttribute('href'), 'href', el);
    });

    root.querySelectorAll('link[rel="modulepreload"]').forEach(el => {
      registerAsset(el.getAttribute('href'), 'href', el);
    });

    root.querySelectorAll('script[src]').forEach(el => {
      registerAsset(el.getAttribute('src'), 'src', el);
    });
    ...
    root.querySelectorAll('video[src]').forEach(el => {
      registerAsset(el.getAttribute('src'), 'src', el);
    });
    root.querySelectorAll('video[poster]').forEach(el => {
      registerAsset(el.getAttribute('poster'), 'poster', el);
    });
    ...
    fs.writeFileSync(finalHtmlPath, root.toString());
