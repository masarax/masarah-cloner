...
`;
  ...
  domains.forEach((d, idx) => {
    ...
    // Express middleware to serve downloaded offline API assets first, falling back to proxy
    serverCode += `// Serve local static JSON API data if downloaded on disk\n`;
    serverCode += `const fs = require('fs');\n`;
    serverCode += `${appName}.get('/api/:endpoint', (req, res, next) => {\n`;
    serverCode += `  const cleanPath = 'api_' + req.params.endpoint + '.json';\n`;
    serverCode += `  const localFile = path.join(DIST_DIR, '${d.name}', cleanPath);\n`;
    serverCode += `  if (fs.existsSync(localFile)) {\n`;
    serverCode += `    return res.sendFile(localFile);\n`;
    serverCode += `  }\n`;
    serverCode += `  next();\n`;
    serverCode += `});\n\n`;

    serverCode += `${appName}.get('/api/:endpoint/:sub', (req, res, next) => {\n`;
    serverCode += `  const cleanPath = 'api_' + req.params.endpoint + '_' + req.params.sub + '.json';\n`;
    serverCode += `  const localFile = path.join(DIST_DIR, '${d.name}', cleanPath);\n`;
    serverCode += `  if (fs.existsSync(localFile)) {\n`;
    serverCode += `    return res.sendFile(localFile);\n`;
    serverCode += `  }\n`;
